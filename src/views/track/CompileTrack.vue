            ...
            <el-form-item label="路线/点位" label-width="80px">
              <div class="item_line">
                ...
                >
                  ...
                  <el-input
                    class="point2"
                    @focus="ChoosePoint('中间点', item.pointName, index)"
      ...
      <el-form :model="AidsList">
        ...
        <div class="Aid_table">
          ...
          >
            ...
            <el-table-column prop="pointName" label="地址"></el-table-column>
            <el-table-column
...
export default {
  ...
  methods: {
    ...
    getTrackDetail() {
      ...
      selectInspectionInfo(param).then((res) => {
        let { head, data } = res;
        if (head.code == 200) {
            this.AidForm.Code = data.rouCode
            this.AidForm.Name= data.rouName
            this.AidForm.timeRange = data.useTime
            let NewArr = data.routePointVo
            //删除第一个元素
            let FirstPoint = NewArr.shift()
            //删除最后一个元素
            let LastPoint = NewArr.pop()
            this.AidForm.origin = FirstPoint.pointName
            this.AidForm.originId = FirstPoint.pointId
            this.AidForm.destination = LastPoint.pointName
            this.AidForm.destinationId = LastPoint.pointId;
    ...
    checkstatus(row) {
      this.tableData = this.tableData.map((item) => {
        if (item.pointId == row.pointId) {
          ...
          if (this.status == "中间点") {
            this.CopyPointList[this.arrNum].pointName = item.pointName;
    ...
    //添加点位
    addPoint() {
      this.PointList.push({ pointName: "", pointId: "" });
      this.CopyPointList.push({ pointName: "", pointId: "" });
    },
    //删除点位
    deletePoint(i) {
    ...
    confirmHandle() {
      if (this.status == "起点") {
        ...
        this.PointList[this.arrNum].pointName = this.CopyPointList[this.arrNum].pointName;
    ...
    confirmAllHandle() {
      ...
      updateInspection(param).then((res) => {
        let { head, data } = res;
        if (head.code == 200) {
          this.$message({
            type: "success",
